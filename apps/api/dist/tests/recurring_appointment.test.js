...
(0, vitest_1.describe)('Recurring Appointment Integration', () => {
    let tenantId;
    let serviceId;
    let staffId;
    ...
    (0, vitest_1.it)('should create a weekly series of 4 appointments on correct 4 Mondays', async () => {
        // Find next Monday
        ...
        (0, vitest_1.expect)(res.body.appointments).toHaveLength(4);
        const appointments = res.body.appointments;
        for (let i = 0; i < 4; i++) {
            const expectedDate = new Date(startDate);
            expectedDate.setUTCDate(startDate.getUTCDate() + (i * 7));
            (0, vitest_1.expect)(new Date(appointments[i].startTimeUtc).toISOString()).toBe(expectedDate.toISOString());
            (0, vitest_1.expect)(appointments[i].seriesIndex).toBe(i);
        }
    });
    (0, vitest_1.it)('should return conflict error if one slot in series is taken', async () => {
        const startDate = new Date();
        ...
        const conflictCustomer = await prisma_1.prisma.customer.create({
            data: {
                tenantId,
        ...
        (0, vitest_1.expect)(res.body.error).toBe('Slot collision in series');
        (0, vitest_1.expect)(res.body.conflictingDate).toBe(thirdWeekDate.toISOString());
    });
    (0, vitest_1.it)('should cancel only one occurrence with scope=single', async () => {
        ...
        const cancelRes = await (0, supertest_1.default)(app_1.app)
            .delete(`/api/appointments/${app2Id}?scope=single`)
            .set('x-tenant-id', tenantId)
            .set('x-user-id', 'test-admin')
            .set('x-user-role', 'ADMIN');
        (0, vitest_1.expect)(cancelRes.status).toBe(200);
        const app1 = await prisma_1.prisma.appointment.findUnique({ where: { id: res.body.appointments[0].id } });
        const app2 = await prisma_1.prisma.appointment.findUnique({ where: { id: app2Id } });
        const app3 = await prisma_1.prisma.appointment.findUnique({ where: { id: res.body.appointments[2].id } });
        (0, vitest_1.expect)(app1?.status).toBe('BOOKED');
        (0, vitest_1.expect)(app2?.status).toBe('CANCELLED');
        (0, vitest_1.expect)(app3?.status).toBe('BOOKED');
    });
    (0, vitest_1.it)('should cancel all future occurrences with scope=series', async () => {
        const start = new Date();
        ...
        (0, vitest_1.expect)(cancelRes.status).toBe(200);
        for (let i = 0; i < 5; i++) {
            const app = await prisma_1.prisma.appointment.findUnique({ where: { id: res.body.appointments[i].id } });
            if (i < 2)
                (0, vitest_1.expect)(app?.status).toBe('BOOKED');
            else
                (0, vitest_1.expect)(app?.status).toBe('CANCELLED');
        }
    });
    (0, vitest_1.it)('should reschedule all future occurrences with scope=series', async () => {
        const start = new Date();
        ...
        const reschedRes = await (0, supertest_1.default)(app_1.app)
        ...
        (0, vitest_1.expect)(reschedRes.status).toBe(200);
        for (let i = 0; i < 3; i++) {
            const app = await prisma_1.prisma.appointment.findUnique({ where: { id: res.body.appointments[i].id } });
            const expectedStart = new Date(start.getTime() + (i * 7 * 24 * 60 * 60 * 1000) + 2 * 60 * 60 * 1000);
            (0, vitest_1.expect)(app?.startTimeUtc.toISOString()).toBe(expectedStart.toISOString());
        }
    });
});
