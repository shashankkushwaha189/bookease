...
exports.cleanupTenantCache = exports.tenantMiddleware = void 0;
...
const tenantMiddleware = async (req, res, next) => {
    ...
    try {
        ...
        logger_1.logger.error({ error, tenantId }, 'Error resolving tenant');
        res.status(500).json({
            success: false,
            error: {
...
async function getTenantById(id) {
    ...
    try {
        ...
        logger_1.logger.error({ error, tenantId: id }, 'Error fetching tenant by ID');
        return null;
    }
}
async function getTenantBySlug(slug) {
    ...
    try {
        ...
        logger_1.logger.error({ error, slug }, 'Error fetching tenant by slug');
        return null;
    }
}
async function getTenantByDomain(domain) {
    ...
    try {
        ...
        logger_1.logger.error({ error, domain }, 'Error fetching tenant by domain');
        return null;
    }
}
// Cache cleanup function (run periodically)
const cleanupTenantCache = () => {
    const now = Date.now();
...
setInterval(exports.cleanupTenantCache, 5 * 60 * 1000);
