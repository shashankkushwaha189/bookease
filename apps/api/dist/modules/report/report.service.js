...
class ReportService {
    ...
    async getSummary(query) {
        ...
        const where = {
            ...
            startTimeUtc: {
                ...
                lte: (0, date_fns_1.endOfDay)(toDate),
            },
        };
        if (serviceId)
     ...
     */
    async getStaffUtilization(query) {
        const startTime = Date.now();
        const { tenantId, fromDate, toDate } = query;
        logger_1.logger.debug({
            tenantId,
        ...
        const staffList = await prisma_1.prisma.staff.findMany({
            ...
            include: {
                ...
                appointments: {
                    where: {
                        startTimeUtc: { gte: (0, date_fns_1.startOfDay)(fromDate), lte: (0, date_fns_1.endOfDay)(toDate) },
                        status: { not: 'CANCELLED' },
                    },
                },
            },
     ...
     */
    async validateCsvExport(csvData, type, tenantId) {
        const issues = [];
        try {
