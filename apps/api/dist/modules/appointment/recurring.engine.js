...
exports.RecurringAppointmentEngine = void 0;
const recurring_schema_1 = require("./recurring.schema");
const prisma_1 = require("../../lib/prisma");
class RecurringAppointmentEngine {
    metrics = {
        totalSeries: 0,
        activeSeries: 0,
        pausedSeries: 0,
    ...
    async createRecurringSeries(data) {
        ...
        try {
            ...
            const validated = recurring_schema_1.createRecurringSeriesSchema.parse(data);
            ...
            const result = await prisma_1.prisma.$transaction(async (tx) => {
                ...
                const series = await tx.recurringSeries.create({
                    data: {
                        ...
                        recurrenceRule: validated.recurrenceRule,
                        status: recurring_schema_1.SeriesStatus.ACTIVE,
                ...
                const appointments = await Promise.all(initialBatch.map(async (occurrence, index) => {
    ...
    async generateRecurrenceDates(data) {
        const startTime = Date.now();
        ...
        while ((!endDate || currentDate <= endDate) &&
            ...
            switch (recurrenceRule.pattern) {
                case recurring_schema_1.RecurrencePattern.WEEKLY:
                    ...
                    break;
                case recurring_schema_1.RecurrencePattern.BI_WEEKLY:
                    this.processWeeklyRecurrence(currentDate, recurrenceRule, baseStart, baseEnd, occurrences);
                    currentDate.setDate(currentDate.getDate() + 14 * recurrenceRule.interval);
                    break;
                case recurring_schema_1.RecurrencePattern.MONTHLY:
        ...
        const filteredOccurrences = occurrences.filter(occ => !exceptionDates.has(occ.startTime.split('T')[0]));
        const generationTime = Date.now() - startTime;
        console.log(`Generated ${filteredOccurrences.length} occurrences in ${generationTime}ms`);
        return filteredOccurrences;
    }
    // Process weekly recurrence
    processWeeklyRecurrence(currentDate, recurrenceRule, baseStart, baseEnd, occurrences) {
    ...
    processMonthlyRecurrence(currentDate, recurrenceRule, baseStart, baseEnd, occurrences) {
        if (recurrenceRule.monthlyType === recurring_schema_1.MonthlyRecurrenceType.DAY_OF_MONTH) {
        ...
        else if (recurrenceRule.monthlyType === recurring_schema_1.MonthlyRecurrenceType.DAY_OF_WEEK) {
            ...
            const targetDate = this.findNthDayOfWeekInMonth(currentDate.getFullYear(), currentDate.getMonth(), targetDayOfWeek, weekOfMonth);
            if (targetDate) {
                targetDate.setHours(baseStart.getHours(), baseStart.getMinutes(), 0, 0);
                const endTime = new Date(targetDate);
                endTime.setHours(baseEnd.getHours(), baseEnd.getMinutes(), 0, 0);
    ...
    findNthDayOfWeekInMonth(year, month, dayOfWeek, weekOfMonth) {
    ...
    async editRecurringSeries(data) {
        try {
            const validated = recurring_schema_1.editRecurringSeriesSchema.parse(data);
            const series = await prisma_1.prisma.recurringSeries.findUnique({
            ...
            if (validated.editMode === 'ENTIRE_SERIES') {
                // Edit entire series
                const updatedSeries = await prisma_1.prisma.$transaction(async (tx) => {
                    // Update series
                    const updated = await tx.recurringSeries.update({
                        ...
                        data: {
                            ...
                            ...(validated.recurrenceRule && { recurrenceRule: validated.recurrenceRule }),
                            updatedAt: new Date(),
                ...
                const newSeries = await prisma_1.prisma.$transaction(async (tx) => {
                    ...
                    const newSeriesData = {
                        ...
                        recurrenceRule: validated.recurrenceRule || series.recurrenceRule,
                        status: recurring_schema_1.SeriesStatus.ACTIVE,
                    ...
                    await Promise.all(occurrences.slice(0, 10).map(occurrence => tx.appointment.create({
                        data: {
    ...
    async cancelRecurringAppointment(data) {
        try {
            const validated = recurring_schema_1.cancelRecurringAppointmentSchema.parse(data);
            if (validated.cancelMode === 'SINGLE') {
                ...
                if (appointment?.seriesId) {
                    await prisma_1.prisma.recurringSeries.update({
                ...
                const cancelledCount = await prisma_1.prisma.appointment.updateMany({
                    where: {
                ...
                await prisma_1.prisma.recurringSeries.update({
                    where: { id: appointment.seriesId },
                    data: {
                        status: recurring_schema_1.SeriesStatus.CANCELLED,
                        cancelledOccurrences: { increment: cancelledCount.count },
                    },
                });
            }
    ...
    async getSeriesWithOccurrences(seriesId) {
        const series = await prisma_1.prisma.recurringSeries.findUnique({
            where: { id: seriesId },
            include: {
                appointments: {
    ...
    updateAverageOccurrencesPerSeries() {
    ...
    async regenerateFutureOccurrences(tx, series, effectiveDate) {
        ...
        const futureOccurrences = occurrences.filter(occ => new Date(occ.startTime) >= new Date(effectiveDate));
        await Promise.all(futureOccurrences.slice(0, 10).map(occurrence => tx.appointment.create({
            data: {
                seriesId: series.id,
                staffId: series.staffId,
                serviceId: series.serviceId,
...
exports.RecurringAppointmentEngine = RecurringAppointmentEngine;
