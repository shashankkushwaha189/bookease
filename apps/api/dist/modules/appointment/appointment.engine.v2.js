...
async;
updateAppointment(appointmentId, string, data, appointment_schema_1.UpdateAppointmentData);
Promise < any > {
    const: startTime = Date.now(),
    try: {
        // Validate request data
...
async;
rescheduleAppointment(data, appointment_schema_1.RescheduleAppointmentData);
Promise < any > {
    const: startTime = Date.now(),
    try: {
        // Validate request data
...
async;
createManualBooking(data, appointment_schema_1.ManualBookingData);
Promise < any > {
    const: startTime = Date.now(),
    try: {
        // Validate request data
...
async;
checkAppointmentConflict(staffId, string, startTimeUtc, string, endTimeUtc, string, excludeAppointmentId, string | null);
Promise < appointment_schema_1.AppointmentConflict > {
...
validateStatusTransition(fromStatus, appointment_schema_1.AppointmentStatus, toStatus, appointment_schema_1.AppointmentStatus | undefined);
appointment_schema_1.StatusTransition;
{
    if (!toStatus) {
        return {
            fromStatus,
...
async;
findAvailableSlots(staffId, string, startTime, Date, endTime, Date, conflictingAppointments, any[]);
Promise < any[] > {
    ...
    for(, conflict, of, sortedConflicts) {
...
async;
recordBookingAttempt(data, (Omit));
Promise < void  > {
    const: attemptData = {
        ...data,
        attemptAt: new Date().toISOString(),
    },
    await, prisma: prisma_1.prisma, : .bookingAttempt.create({
        data: attemptData,
    })
};
async;
getTenantId(staffId, string);
Promise < string > {
...
updateAverageBookingTime(newTime, number);
void {
