...
export class ReportingArchivalEngine {
  private metrics = {
  ...
  async configureArchival(config: ArchivalConfig): Promise<ArchivalConfig> {
    const validatedConfig = archivalConfigSchema.parse(config);

    // Check if configuration exists
    const existing = await prisma.archivalConfig.findFirst({
      where: { tenantId: validatedConfig.tenantId },
    });

    if (existing) {
      const updated = await prisma.archivalConfig.update({
        where: { id: existing.id },
        data: validatedConfig,
      });
      return updated as ArchivalConfig;
    } else {
      const created = await prisma.archivalConfig.create({
        data: validatedConfig,
      });
      return created as ArchivalConfig;
    }
  }

  // Get archival configuration
  async getArchivalConfig(tenantId: string): Promise<ArchivalConfig | null> {
    const config = await prisma.archivalConfig.findFirst({
      where: { tenantId, enabled: true },
    });
    return config as ArchivalConfig | null;
  }

  // Run archival job
  async runArchivalJob(tenantId: string): Promise<ArchivalJob> {
    ...
    // Create archival job
    const job = await prisma.archivalJob.create({
      data: {
        tenantId,
        status: 'RUNNING',
        startedAt: new Date(),
  ...
  async searchArchivedAppointments(search: ArchiveSearch): Promise<ArchiveResult> {
    const validatedSearch = archiveSearchSchema.parse(search);
    ...
    if (validatedSearch.startDate || validatedSearch.endDate) {
      where.startTimeUtc = {};
      if (validatedSearch.startDate) where.startTimeUtc.gte = validatedSearch.startDate;
      if (validatedSearch.endDate) where.startTimeUtc.lte = validatedSearch.endDate;
    }
  ...
  async getArchivalJob(jobId: string): Promise<ArchivalJob | null> {
  ...
  // Get archival jobs
  async getArchivalJobs(tenantId: string): Promise<ArchivalJob[]> {
    const jobs = await prisma.archivalJob.findMany({
      where: { tenantId },
  ...
  private async generateServiceReport(query: ReportQuery): Promise<ServiceReportData[]> {
  ...
  private async generateStaffReport(query: ReportQuery): Promise<StaffReportData[]> {
    const where = this.buildWhereClause(query);
    ...
    return Object.values(staffData).map(staff => ({
      ...
      topServices: Object.entries(staff.services)
        .map(([serviceName, count]) => ({ serviceName, count: count as number }))
        .sort((a, b) => b.count - a.count)
        .slice(0, 5)
        .map((svc, index) => ({
  ...
  private async generateNoShowReport(query: ReportQuery): Promise<NoShowReportData> {
    const where = this.buildWhereClause(query);
    ...
    const byService = appointments.reduce((acc, apt) => {
      ...
      acc[serviceId].total++;
      if (apt.status === 'NO_SHOW') acc[serviceId].noShows++;
      return acc;
    }, {} as Record<string, any>);
    ...
    const byStaff = appointments.reduce((acc, apt) => {
      ...
      acc[staffId].total++;
      if (apt.status === 'NO_SHOW') acc[staffId].noShows++;
      return acc;
    }, {} as Record<string, any>);
    ...
    const byTimeOfDay = appointments.reduce((acc, apt) => {
      ...
      acc[hour].total++;
      if (apt.status === 'NO_SHOW') acc[hour].noShows++;
      return acc;
    ...
    const byDayOfWeek = appointments.reduce((acc, apt) => {
      ...
      acc[dayOfWeek].total++;
      if (apt.status === 'NO_SHOW') acc[dayOfWeek].noShows++;
      return acc;
    }, {} as Record<number, any>);
  ...
  private async generatePeakTimesReport(query: ReportQuery): Promise<PeakTimesReportData> {
    const where = this.buildWhereClause(query);
  ...
  private buildWhereClause(query: ReportQuery): any {
    const where: any = {
  ...
  private exportToCSV(data: any, request: ExportRequest): string {
  ...
  private async processArchivalJob(jobId: string, config: ArchivalConfig): Promise<void> {
    try {
  ...
  private async generateArchiveSummary(tenantId: string, where: any): Promise<any> {
    const [totalArchived, dateRange, statusBreakdown] = await Promise.all([
      prisma.appointmentArchive.count({ where }),
      prisma.appointmentArchive.aggregate({
        where,
  ...
  private generateRecommendations(hourlyData: any[], appointments: any[]): any[] {
  ...
  private updateAverageReportGenerationTime(newTime: number): void {
