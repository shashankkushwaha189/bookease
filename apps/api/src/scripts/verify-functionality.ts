...
class FunctionalityVerifier {
  ...
  private apiBaseUrl = 'http://localhost:3000'; // Changed from 3001 to 3000
  private webBaseUrl = 'http://localhost:3000';
  private authToken: string = '';
  private tenantId = 'b18e0808-27d1-4253-aca9-453897585106'; // Using actual tenant ID instead of slug
  ...
  private async verifySystemHealth(): Promise<void> {
    ...
    try {
      const response = await axios.get(`${this.apiBaseUrl}/ready`);
  ...
  private async verifyDemoData(): Promise<void> {
  ...
  private async verifyAuthentication(): Promise<void> {
    ...
    try {
      // Test Admin Login
      const adminLogin = await axios.post(`${this.apiBaseUrl}/api/auth/login`, {
      ...
      // Test Invalid Login
      try {
        await axios.post(`${this.apiBaseUrl}/api/auth/login`, {
  ...
  private async verifyCoreFunctionality(): Promise<void> {
    console.log('⚙️ Step 5: Core Functionality Verification');

    try {
      // Test creating a service
      const serviceResponse = await axios.post(`${this.apiBaseUrl}/api/services`, {
        name: 'Test Service',
        durationMinutes: 30,
        price: 100
      }, {
      ...
      const servicesResponse = await axios.get(`${this.apiBaseUrl}/api/services`, {
        headers: {
          'Authorization': `Bearer ${this.authToken}`,
          'X-Tenant-ID': this.tenantId
        }
      ...
      if (servicesResponse.data && servicesResponse.data.data && servicesResponse.data.data.length > 0) {
        ...
        const bookingResponse = await axios.post(`${this.apiBaseUrl}/api/public/bookings`, {
      ...
      // Test staff listing
      const staffResponse = await axios.get(`${this.apiBaseUrl}/api/staff`, {
        headers: {
          'Authorization': `Bearer ${this.authToken}`,
          'X-Tenant-ID': this.tenantId
        }
  ...
  private async verifyAIFeatures(): Promise<void> {
    console.log('🤖 Step 6: AI Features Verification');

    try {
      // Test AI configuration (AI routes are under /api/appointments)
      const configResponse = await axios.get(`${this.apiBaseUrl}/api/appointments/configuration`, {
        headers: {
          'Authorization': `Bearer ${this.authToken}`,
          'X-Tenant-ID': this.tenantId
        }
      ...
      try {
        const appointmentsResponse = await axios.get(`${this.apiBaseUrl}/api/appointments`, {
          headers: {
            'Authorization': `Bearer ${this.authToken}`,
            'X-Tenant-ID': this.tenantId
          }
        ...
        if (appointmentsResponse.data && appointmentsResponse.data && appointmentsResponse.data.length > 0) {
          const completedAppointment = appointmentsResponse.data.find((apt: any) => apt.status === 'COMPLETED');
          
          if (completedAppointment) {
            const summaryResponse = await axios.post(`${this.apiBaseUrl}/api/appointments/summaries/${completedAppointment.id}`, {}, {
              headers: {
                'Authorization': `Bearer ${this.authToken}`,
                'X-Tenant-ID': this.tenantId
              }
  ...
  private async verifyPerformance(): Promise<void> {
    ...
    try {
      ...
      await axios.get(`${this.apiBaseUrl}/api/services`, {
        headers: {
          'Authorization': `Bearer ${this.authToken}`,
          'x-tenant-id': this.tenantId
        }
      ...
      const promises = Array.from({ length: concurrentRequests }, () => 
        axios.get(`${this.apiBaseUrl}/health`)
      );
...
export { FunctionalityVerifier, runVerificationCommand };
