...
import { UserRole, AppointmentStatus, AiConfidence } from '@prisma/client';
...
export class DemoSeeder {
  ...
  private static readonly DEMO_SERVICES = [
    {
  ...
  private static readonly DEMO_STAFF = [
    {
  ...
  private static readonly DEMO_CUSTOMERS = [
    {
      name: 'John Anderson',
  ...
  private static readonly DEMO_APPOINTMENTS = [
    {
      referenceId: 'BK-001',
  ...
  private static readonly DEMO_AI_SUMMARIES = [
    {
  ...
  static async seedDemoData(): Promise<DemoSeedData> {
  ...
  static async cleanDemoData(): Promise<void> {
  ...
  static async resetDemoData(): Promise<DemoSeedData> {
  ...
  static async getDemoDataStatus(): Promise<{
    exists: boolean;
    tenant?: any;
    counts: {
      users: number;
    ...
    try {
      ...
      logger.error('Failed to get demo data status', { error });
      throw error;
    }
  }

  static async verifyDemoData(): Promise<{
    valid: boolean;
    issues: string[];
    summary: {
      tenant: boolean;
    ...
    try {
      ...
      logger.error('Failed to verify demo data', { error });
...
export async function seedDemoCommand() {
  try {
...
export async function statusDemoCommand() {
  try {
