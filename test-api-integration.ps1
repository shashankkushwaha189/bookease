...
try {
    ...
    $availabilityResponse = Invoke-WebRequest -Method GET -Uri "$baseUrl/api/public/availability" -Headers $headers -Body $availabilityData
...
if ($token) {
    ...
    try {
        ...
        # Availability
        $availabilityResponse = Invoke-WebRequest -Method GET -Uri "$baseUrl/api/availability?serviceId=c45a5e05-78d8-4164-a1ac-313e2cefcfce&staffId=68f11f56-fdc5-4070-8e0a-09fc33506ded&date=2026-03-14" -Headers $authHeaders
        Write-Host "   ✅ Availability: $($availabilityResponse.StatusCode)"
        
        # Reports
        $reportsResponse = Invoke-WebRequest -Method GET -Uri "$baseUrl/api/reports?from=2026-03-01&to=2026-03-31" -Headers $authHeaders
